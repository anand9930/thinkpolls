$(document).ready(function() {
  ...
  $.ajax({
    ...
    success: function(json) {
      ...
      if (total_votes == 0) {
        ...
        document.getElementById("total-votes").innerText = total_votes;
      } else {
        for (i = 0; i < keyCount; i++) {
          $(".form-block").append(
            '<div class="label-wrapper"><label class="option">' +
              ...
              '%" ></div></label><div class="radio-block">' +
              ((jsonObject[i].votes * 100) / total_votes).toFixed(0) +
              "<span>%</span><div class='result-votes''><div>" +
              jsonObject[i].votes +
              "</div><span>votes</span></div></div> </div>"
          );
          console.log(jsonObject[i].option);
        }
  ...
  $(".poll-submit-button").click(function() {
    $("#share-option-block").css({
      opacity: "1",
      visibility: "visible",
      height: "85%"
    });
    $("#share-option-block div").css({
      transform: "scale(1)"
    });
  });
  /* $("#share-option-block").mouseleave(function() {
    $("#share-option-block")
      .delay(1000)
      .css({
        opacity: "0",
