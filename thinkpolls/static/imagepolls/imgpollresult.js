$(document).ready(function () {
  ...
  $("#embed").click(function () {
    ...
    btn.setAttribute("onclick", "submitvote()");
    btn.innerHTML = "vote";
    ...
    script_src.setAttribute(
      "src",
      "https://thinkpolls.herokuapp.com/static/poll_detail/embed.js"
    );
  ...
  $.ajax({
    ...
    success: function (json) {
      // $(".question-block").html(json.question);
      $(".question-block").text(json.question);
      console.log(json);
      var imgoptionbject = json.imageoptions;
      var option_count = Object.keys(imgoptionbject).length;
      console.log(option_count);
      var total_votes = 0;
      var vote_counter = 0;
      for (i = 0; i < option_count; i++) {
        vote_counter = parseInt(imgoptionbject[i].votes);
        total_votes = total_votes + vote_counter;
      }
      if (option_count % 2 == 0) {
        for (i = 0; i <= option_count / 2; i = i + 2) {
          $("#addimg-wrapper").prepend(
            '<div class="label-grid-wrapper"><div class="label-wrapper  a"><label id="option"><img src="' +
            imgoptionbject[i].image_option_url +
            '" /><div class="image-results-block"><div class="result-bar-wrapper"><div class="result-bar" style="width:' +
            (imgoptionbject[i].votes / total_votes) * 100 +
            '%"/></div><div class="result-percent">' +
            (imgoptionbject[i].votes / total_votes).toFixed(2) * 100 +
            '<span>%</span></div></div><div class="result-votes"><div>' +
            imgoptionbject[i].votes +
            '</div><span>votes</span></div></label></div><div class="label-wrapper b"><label id="option"><img src="' +
            imgoptionbject[i + 1].image_option_url +
            '" /><div class="image-results-block"><div class="result-bar-wrapper"><div class="result-bar" style="width:' +
            (imgoptionbject[i + 1].votes / total_votes) * 100 +
            '%" ></div></div><div class="result-percent">' +
            (imgoptionbject[i + 1].votes / total_votes).toFixed(2) * 100 +
            "<span>%</span></div></div> <div class='result-votes'><div>" +
            imgoptionbject[i + 1].votes +
            "</div><span>votes</span></div>  </label></div></div>"
          );
          console.log(imgoptionbject[i].image_option_url);
          console.log(imgoptionbject[i + 1].image_option_url);
        }
      } else {
        $("#addimg-wrapper").prepend(
          '<div class="label-grid-wrapper"><div class="label-wrapper  a"><label id="option"><img src="' +
          imgoptionbject[option_count - 1].image_option_url +
          '" /><div class="image-results-block"><div class="result-bar-wrapper"><div class="result-bar" style="width:' +
          (imgoptionbject[option_count - 1].votes / total_votes) * 100 +
          '%"/></div><div class="result-percent">' +
          (imgoptionbject[option_count - 1].votes / total_votes).toFixed(2) *
          100 +
          "<span>%</span></div></div><div class='result-votes'><div>" +
          imgoptionbject[option_count - 1].votes +
          "</div><span>votes</span></div> </label></div></div>"
        );

        for (i = 0; i <= (option_count - 1) / 2; i = i + 2) {
          console.log("hello");
          $("#addimg-wrapper").prepend(
            '<div class="label-grid-wrapper"><div class="label-wrapper  a"><label id="option"><img src="' +
            imgoptionbject[i].image_option_url +
      ...
      for (i = 0; i < keyCount; i++) {
        vote_counter = parseInt(jsonObject[i].votes);
        total_votes = total_votes + vote_counter;
      }
      ...
      /* if (total_votes == 0) {
        for (i = 0; i < keyCount; i++) {
          $(".form-block").append(
            '<div class="label-wrapper"><label class="option">' +
              jsonObject[i].option +
              '<div class="label-background" style="width:' +
              0 +
              '%" ></div></label><div class="radio-block">' +
              0 +
              "%</div> </div>"
          );
          console.log(jsonObject[i].option);
        }
      } else {
        for (i = 0; i < keyCount; i++) {
          $(".form-block").append(
            '<div class="label-wrapper"><label class="option">' +
              jsonObject[i].option +
              '<div class="label-background" style="width:' +
              (jsonObject[i].votes / total_votes) * 100 +
              '%" ></div></label><div class="radio-block">' +
              (jsonObject[i].votes / total_votes).toFixed(2) * 100 +
              "%</div> </div>"
          );
          console.log(jsonObject[i].option);
        }
